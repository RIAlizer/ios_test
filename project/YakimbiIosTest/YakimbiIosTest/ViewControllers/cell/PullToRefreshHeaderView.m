...
- (id)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame])
	{
		...
		RELEASE(lastUpdatedLabel);
		
		statusLabel = [[UILabel alloc] initWithFrame:CGRectMake(0.0f,
																frame.size.height - 48.0f, frame.size.width, 20.0f)];
...
- (void)setLastUpdatedDate:(NSDate *)newDate
{
	if (newDate)
	{
		...
		{
			RELEASE(lastUpdatedDate);;
		}
		
		//lastUpdatedDate = [newDate retain];
		lastUpdatedDate = [NSDate dateWithTimeIntervalSince1970:[newDate timeIntervalSince1970]];
